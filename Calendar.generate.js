...
(function (Calendar) {
    ...
    var regen_main = function (calendar, deltaFrequency, start, end, empty) {
        ...
        if (!currentIsMain && control.init) {
            ...
            var condition = true
            var reverse = true
            for (var f = _first; condition; f+=iterate) {
                // REVERSE
                if (reverse) {
                    condition = f >= first
                ...
                if (!element.list) {
                    ...
                    if (element.wait) {
                        //debugger
                        //control.list[f][0] = control.list[fbLen-1][0]
                        label = '-'
                    }
                    ...
                    //
                    if (reverse)
                        parentSpan.appendAfter(elemFreqSpan, parentSpan.children[1])
                    else
                        parentSpan.appendBeforeLastChild(elemFreqSpan)
                ...
                if (element.wait) {
                    for (var w = elemFreqSpanBandDiv.children.length; w < 2; w++) {
                ...
                for (var b = elemFreqSpanBandDiv.children.length; b < _nbLen; b++) {
                    var elemFreqSpanBandDivUnit = document.createElement('div')
                    if (b == 0)
                        elemFreqSpanBandDivUnit.setAttribute('class', 'first')
                    if (b >= nbLen)
                        elemFreqSpanBandDivUnit.setAttribute('class', 'wait')
                    elemFreqSpanBandDiv.appendChild(elemFreqSpanBandDivUnit)
                }
                elemFreqSpan.appendChild(elemFreqSpanBandDiv)
        ...
        for (var p = first; p < length; p++) {// days and hours problem to solve 
            // Todo - For Calendar Days shifting p++ and pbLen required
            ...
            for (var f = fChildSpan; f < _fbLen; f++) {// days and hours problem to solve 
                ...
                if (!element.list) {
                    ...
                    if (element.wait) {
                        //debugger
                        container.list[f][0] = container.list[fbLen-1][0]
                        label = '-'
                    }
                    ...
                    if (calendar.ctx.style[deltaFrequency.name])
                        parentSpan.appendBeforeLastChild(elemFreqSpan)
                    else
            ...
            if (!control.init && !currentIsMain) {
                ...
                for (var li = 0; li < container.list.length; li++) {
        ...
        if (!control.init) {
            ...
            if (currentIsMain) 
            control.list.parent = ['timeline-code', {span: parentSpan}]
            else
            control.list.parent = calendar.stack[previousFrequencyName]
    ...
    var R = {
        ...
        day: regen_main,
        morningNoon: function morningNoon(calendar, element, name, first, iterate, subFreq, subLabel) {
            if (element[name] || !element.enter)
                return
        ...
        hour: regen_main,
        tenMinutes: function tenMinutes(calendar, element, name, first, iterate, subFreq, subLabel) {
            if (element[name] || !element.enter)
                return
        ...
        minute: regen_main,
        tenSeconds: function tenSeconds(calendar, element, name, first, iterate, subFreq, subLabel) {
            if (element[name] || !element.enter)
                return
    ...
    var degen_main = function (calendar, deltaFrequency, start, end, empty) {
        ...
        var toggleFrequencyStyle = calendar.ctx.style.toggle
        ...
        if (control.empty != empty) {
            ...
            if (deltaFrequencyStyle.fill.left.enter)
            deltaFrequencyStyle.fill.left.span.parentNode.removeChild(deltaFrequencyStyle.fill.left.span);
            deltaFrequencyStyle.fill.left.enter = false
            if (deltaFrequencyStyle.fill.right.enter)
            deltaFrequencyStyle.fill.right.span.parentNode.removeChild(deltaFrequencyStyle.fill.right.span);
        ...
        var _cbLen = deltaFrequency._bands
        var deltaPrevLocator = deltaFrequency.range.locator / deltaFrequency._bands
        deltaPrevLocator = deltaPrevLocator < 1 ? 1 : deltaPrevLocator > previousStack.list.length-1 ? previousStack.list.length-1 : deltaPrevLocator
    ...
    that.Init = function (calendar, bandWidths) {
        ...
        this.reset = function (_current, _delta, _next, start, end, subReset) {
            ...
            if (currentFrequencyStyle.fill.left.enter)
            currentFrequencyStyle.fill.left.span.parentNode.removeChild(currentFrequencyStyle.fill.left.span);
            currentFrequencyStyle.fill.left.enter = false
            if (currentFrequencyStyle.fill.right.enter)
            currentFrequencyStyle.fill.right.span.parentNode.removeChild(currentFrequencyStyle.fill.right.span);
            currentFrequencyStyle.fill.right.enter = false

            var deltaFrequencyStyle = calendar.ctx.style[_delta]
            var deltaFrequency = calendar.phases.frequencies[_delta]
